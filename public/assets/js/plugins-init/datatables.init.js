...
// Variabel yang menyimpan tabel DataTable
var table = $('#tasksTable').DataTable({
    'dom': 'ZBfrltip',
    buttons: [
        {
            extend: 'excel',
            text: '<i class="fa-solid fa-file-excel"></i> Export Tasks',
            className: 'btn btn-sm border-0',
            customize: function (xlsx) {
                var sheet = xlsx.xl.worksheets['sheet1.xml'];
                $('row c[r^="C"]', sheet).each(function () {
                    var imgTags = $(this).find('v img');
                    var imgNames = [];

                    imgTags.each(function () {
                        var imgAlt = $(this).attr('alt');
                        imgNames.push(imgAlt || '');
                    });

                    var imgNamesStr = imgNames.join(', ');
                    $(this).find('v').text(imgNamesStr);
